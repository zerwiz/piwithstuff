...
// Handle memory export operations
export async function handleMemoryExport(
  format: "json" | "text" | "md" | "preview",
  agentName: string,
  cwd?: string,
): Promise<string> {
  try {
    ...
    const memoryDir = cwd ? `.pi/memory-export` : ".pi/memory-export";
    const exportPath = `${memoryDir}/memory-export.${format}`;

    if (format === "preview") {
      return "✅ Memory export preview shown in UI";
    }

    // Return placeholder for actual export
